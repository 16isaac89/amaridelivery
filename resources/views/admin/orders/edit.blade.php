    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="text" name="size" id="size" value="{{ old('size', $order->size) }}" required>
                @if($errors->has('size'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vehicle') ? 'is-invalid' : '' }}" type="text" name="vehicle" id="vehicle" value="{{ old('vehicle', $order->vehicle) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $order->from) }}" required>
                @if($errors->has('from'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('to') ? 'is-invalid' : '' }}" type="text" name="to" id="to" value="{{ old('to', $order->to) }}" required>
                @if($errors->has('to'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('money') ? 'is-invalid' : '' }}" type="text" name="money" id="money" value="{{ old('money', $order->money) }}">
                @if($errors->has('money'))
            ...
            <div class="form-group">
                <label for="distance">{{ trans('cruds.order.fields.distance') }}</label>
                <input class="form-control {{ $errors->has('distance') ? 'is-invalid' : '' }}" type="text" name="distance" id="distance" value="{{ old('distance', $order->distance) }}">
                @if($errors->has('distance'))
                    <span class="text-danger">{{ $errors->first('distance') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.distance_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="datetime">{{ trans('cruds.order.fields.datetime') }}</label>
                <input class="form-control datetime {{ $errors->has('datetime') ? 'is-invalid' : '' }}" type="text" name="datetime" id="datetime" value="{{ old('datetime', $order->datetime) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('route') ? 'is-invalid' : '' }}" name="route_id" id="route_id">
                    @foreach($routes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('route_id') ? old('route_id') : $order->route->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('route'))
